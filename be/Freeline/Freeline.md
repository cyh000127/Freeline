...
- 현재 SSE 대상 waiting 이벤트는 `WAITING_CALLED`, `WAITING_REGISTERED`, `WAITING_ENTERED`, `WAITING_EXITED`, `WAITING_EXPIRED`, `WAITING_CANCELED` 기준으로 관리하며, 실제 publish 여부는 `CALLED` 이후 가시 상태(`FRONT_QUEUE`, `IN_USE`)에 영향을 주는지로 한 번 더 판단한다.
