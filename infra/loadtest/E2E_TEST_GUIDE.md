...
### 방법 1: 통합 실행 스크립트 (run.sh)

Infisical 시크릿 주입 + 테스트 실행을 한번에 처리합니다.

```bash
# Server A에서 실행
cd /home/ubuntu/S14P21A207/infra/loadtest

# E2E 리포트 파이프라인 전체 검증 (기본: VU 50명, 10회 반복)
./run.sh

# 가볍게 실행 (VU 5명, 2회)
VU_COUNT=5 ITERATIONS=2 ./run.sh

# 대규모 부하테스트 (VU 1000명)
VU_COUNT=1000 ITERATIONS=5 ./run.sh
...
> `run.sh`는 `infisical run --env=prod --path=/be`로 `ADMIN_ID`, `ADMIN_PW`, `DB_USERNAME`, `DB_PASSWORD` 등을 자동 주입한 뒤 대상
> 스크립트를 실행합니다.
...
| `ADMIN_ID`    | *(Infisical에서 주입)*           | 관리자 로그인 ID         |
| `ADMIN_PW`    | *(Infisical에서 주입)*           | 관리자 비밀번호           |
| `VU_COUNT`    | `50`                         | 동시 가상 사용자 수        |
| `ITERATIONS`  | `10`                         | VU당 반복 횟수          |
| `DB_USERNAME` | *(Infisical에서 주입)*           | PostgreSQL 사용자     |
| `DB_PASSWORD` | *(Infisical에서 주입)*           | PostgreSQL 비밀번호    |
...
TOKEN=$(curl -s -X POST http://localhost:8080/api/v1/auth/login \
  ...
  -d "{\"id\":\"${ADMIN_ID}\",\"password\":\"${ADMIN_PW}\"}" \
...
```bash
docker exec -e PGPASSWORD="$DB_PASSWORD" freeline-db psql -U "$DB_USER" -d freeline -c "
...
| 파일                                                   | 역할                                      |
|------------------------------------------------------|-----------------------------------------|
| `infra/loadtest/run.sh`                              | 통합 실행 스크립트 (Infisical 시크릿 주입 + 스크립트 실행) |
